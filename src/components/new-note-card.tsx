...
export function NewNoteCard({onNoteCreated}: NewNoteCardProps){
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 text-left p-5 gap-3 overflow-hidden outline-none relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
        <span className="text-sm font-medium text-slate-200">
          Adicionar nota
        </span>
        <p className="text-sm leading-6 text-slate-400">
          Grave um nota em áudio que será convertida em texto automaticamente.
        </p>
      </Dialog.Trigger>
          ...
          <form className="flex-1 flex flex-col"> 
            <div className="flex flex-1 flex-col gap-3 p-5">
              ...
              {shouldShowOnBoarding ? (
                ...
                className="text-lime-400 font-medium hover:underline">gravando uma nota</button> em áudio ou se preferir <button 
                type="button" 
                onClick={handleStartEditor} 
                className="text-lime-400 font-medium hover:underline">utilize apenas texto</button>.
              </p>
              ) : (
                <textarea 
                autoFocus
